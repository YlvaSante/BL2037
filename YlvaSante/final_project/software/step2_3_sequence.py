...
def run_sequence_analysis():
    ...
    # 2. Parsing Clustal Output Matrix
    # Clustals matrix format has protein names in the first column and numbers separated by spaces
    print("Parsing the output matrix...")
    ...
    # Checking whether the matrix contains NaN, replacing them with 0
    clean_pim = pim_df.copy().fillna(0)
    
    # Checking the internal diagonal to dynamically identify if it is a distance or identity matrix
    # If the average value along the diagonal is close to 0, it is an evolutionary distance matrix
    diag_mean = np.mean(np.diag(clean_pim.values))
    print(f"-> Matrix diagnostic - Internal diagonal mean value: {diag_mean:.4f}")
    
    if diag_mean < 5.0:
        print("-> Detected Clustal DISTANCE matrix. Inverting to calculate Identity (100 - distance)...")
        # Checking if the scale is 0-1 or 0-100 before performing inversion
        if clean_pim.values.max() <= 1.0:
            clean_pim = clean_pim * 100
        clean_pim = 100 - clean_pim
    else:
        print("-> Detected Clustal IDENTITY matrix. Keeping raw values...")
        # If it is an identity matrix scaled between 0-1, normalising up to 0-100%
        if clean_pim.values.max() <= 1.0:
            clean_pim = clean_pim * 100

    # Ensuring the matrix is completely symmetric and the diagonal is strictly 100% identity
    clean_pim = (clean_pim + clean_pim.T) / 2
    np.fill_diagonal(clean_pim.values, 100)
    
    # Ensuring no values overflow the absolute 0-100% limits due to floating point precision
    clean_pim = pd.DataFrame(np.clip(clean_pim.values, 0, 100), index=labels, columns=labels)

    # Saving normalized identity matrix as CSV for future use in the report
    ...
    # For hierarchical clustering a DISTANCE matrix is required. Distance = 100 - Percentage identity
    print("Calculating hierarchical clustering for dendrogram...")
    distance_matrix = 100 - clean_pim.values
    np.fill_diagonal(distance_matrix, 0) # Diagonal distance to self must be exactly 0
